...
r = requests.get('http://www.msac.gov.au/internet/msac/publishing.nsf/Content/application-page',
                 headers=headers, timeout=60)
soup = BeautifulSoup(r.content, 'html.parser')
links = []
rows = soup.findAll('td')
for row in rows:
    link = f"http://www.msac.gov.au{row.find('a')['href']}"
    links.append(link)


time.sleep(1)
list = []
for i in range(1000, 1665):
    list.append(i)
...
df1 = pd.DataFrame(s)
df = df1.drop_duplicates(subset=['link'], keep='first').reset_index(drop=True)
df.to_excel('msac_application_links.xlsx', encoding='utf-8', index=False)
# saved in an excel, in-case you want to check the links manually.

time.sleep(1)

df1 = pd.read_excel('msac_application_links.xlsx')
...
df.to_csv('msac_final.csv', encoding='utf-8-sig', index=False)
